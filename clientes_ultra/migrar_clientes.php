...
$resultados = $sqlServer->select("SELECT * FROM data_ultra_raw WHERE flg_migrado = 0 and RUC = '20522093743'");
...
foreach ($resultados as $fila) {
    ...
    else if ( is_numeric($fila['IdPedido']) ) // is_numeric($fila['IdWinforce']) AND
    {
...
//    print_r_f($resultados);
    ...
    {
        print_r_f(["Resultados diferente a 1 :Error 64",$data]);
...
// ]);
    ...
    else
    {
        ...
        FROM  PE_OPTICAL_ADM.ECOM.ECOM_CLIENTE CLI -- ON C.cli_codigo_ecom = CLI.CLII_ID_CLIENTE
        INNER JOIN PE_OPTICAL_ADM.ECOM.ECOM_EMPRESA_CLIENTE EC ON CLI.CLII_ID_CLIENTE = EC.CLII_ID_CLIENTE
        INNER JOIN PE_OPTICAL_ADM.ECOM.ECOM_CONTRATO CO ON EC.EMCI_ID_EMP_CLI = CO.EMCI_ID_EMP_CLI
        INNER JOIN PE_OPTICAL_ADM.ECOM.ECOM_SERVICIO S ON CO.CONI_ID_CONTRATO = S.CONI_ID_CONTRATO -- AND S.SERI_ID_SERVICIO = CIR.cir_codigo_ecom
        ...
        $insertQuery = "INSERT INTO data_ultra_procesado (id_data,
...
function get_representante_by_ruc($ruc)
{
    if($ruc == '20522093743'){
        return [
            ...
            'desc_numero_documento' => '08802548',
            'desc_nombres' => 'MIGUEL ANGEL',
            'desc_apellido_paterno' => 'SALMON',
            'desc_apellido_materno' => 'JACOBS'
        ];
    }
    if($ruc == '20544994779'){
        return [
            'desc_tipo_documento' => 'DNI',
            'desc_numero_documento' => '41553762',
            'desc_nombres' => 'GIANMARCO',
            'desc_apellido_paterno' => 'TAGLIARINO',
            'desc_apellido_materno' => 'RONDINELLI'
        ];
    }
    if($ruc == '20605718362') {
        return [
